...
/*---------------------------------------------------------------------
 * Method: number_of_bits(..) 
 * Scope: local 
 *
 * Counts the number of bits of the unsigned 32bit interger value.
 *
 ---------------------------------------------------------------------*/
size_t number_of_bits(uint32_t bits) {
    size_t count = 0;
...
/*---------------------------------------------------------------------
 * Method: mathced_prefix(..) 
 * Scope: local 
 *
 * Returns if the ip is matched to the given route table's entry. 
 * If matched returns number of bits matched. 
 * If not matched returns 0
 *
 ---------------------------------------------------------------------*/
int matched_prefix(struct sr_rt *rt_entry, uint32_t ip) {
    if (ip == 0) {
        return number_of_bits(rt_entry->mask.s_addr);
    }
 ...
 * Scope:  Global
 *
 * This method performs longest prefix match to find the next hop point/
 * interface for the packet to send.
 *
 ---------------------------------------------------------------------*/
struct sr_rt *longest_prefix_match(struct sr_instance *sr, uint32_t to_match) {
    struct sr_rt* rt_walker; 
    struct sr_rt* longest_prefix = 0;
    int matched_len = 0;

    for (rt_walker = sr->routing_table; rt_walker; rt_walker = rt_walker->next) {
        int tmp = matched_prefix(rt_walker, to_match);
        if (tmp > matched_len) {
            matched_len = tmp;
